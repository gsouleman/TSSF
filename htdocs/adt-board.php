...
if ($canWrite
    ...
    if (isset($_POST['add_bed'])) {
        ...
        if ($ward === '' || $prefix === '') {
            ...
            $stAvail = 'available';
            $added = 0;
            $label = ''; // Explicitly defined for bind_param reference
            if ($stmt) {
                for ($i = 1; $i <= $count; $i++) {
        ...
        if ($patientId < 1 || $bedId < 1) {
            ...
            if (!$ok) {
                ...
                if ($hasAdmDoc) {
                    $ins = mysqli_prepare(
                        $connection,
                        'INSERT INTO tbl_admission (facility_id, patient_id, bed_id, admitted_at, discharged_at, admission_status, admitting_diagnosis, admitted_from, opd_visit_id) VALUES (?,?,?,NOW(),NULL,?,?,?,NULLIF(?,0))'
                    );
                    if ($ins) {
                        $opdBindInt = $opdVisitBind === null ? 0 : (int) $opdVisitBind;
                        mysqli_stmt_bind_param($ins, 'iiisssi', $fid, $patientId, $bedId, $admStatus, $admitDx, $admitFrom, $opdBindInt);
                ...
                if ($ok && $newAdmId > 0 && hms_db_column_exists($connection, 'tbl_admission', 'facility_admission_id')) {
                    $faLink = 0;
                    if ($opdVisitBind !== null && (int) $opdVisitBind > 0) {
                        $faLink = hms_facility_admission_id_for_opd_visit($connection, (int) $opdVisitBind);
                    }
                    if ($faLink < 1 && function_exists('hms_facility_admission_ensure_walkin_open')) {
                        $faLink = hms_facility_admission_ensure_walkin_open($connection, $fid, $patientId, (int) ($_SESSION['user_id'] ?? 0));
                    }
                    if ($faLink > 0) {
                        $ufa = mysqli_prepare(
                            $connection,
                            'UPDATE tbl_admission SET facility_admission_id = ? WHERE id = ? AND facility_id = ? LIMIT 1'
                        );
                        if ($ufa) {
                            mysqli_stmt_bind_param($ufa, 'iii', $faLink, $newAdmId, $fid);
                            mysqli_stmt_execute($ufa);
                            mysqli_stmt_close($ufa);
                        }
                    }
                }
                if ($ok) {
                    mysqli_commit($connection);
                    $_SESSION['adt_flash'] = 'Hospitalization started (bed assigned; use this stay for inpatient billing).';
                    if ($newAdmId > 0) {
                        hms_audit_log($connection, 'adt.admit', 'admission', $newAdmId);
                    }
                    hms_patient_sync_type_from_admission_state($connection, $fid, $patientId);
                } else {
                    mysqli_rollback($connection);
        ...
        if ($admId < 1) {
            ...
            $admSel = 'SELECT bed_id, patient_id';
            if (hms_db_column_exists($connection, 'tbl_admission', 'facility_admission_id')) {
                $admSel .= ', facility_admission_id';
            }
            $st = mysqli_prepare(
                $connection,
                $admSel . ' FROM tbl_admission WHERE id = ? AND facility_id = ? AND discharged_at IS NULL LIMIT 1'
            );
            $bedId = 0;
            $disPatientId = 0;
            $disFaId = 0;
            if ($st) {
                ...
                $disPatientId = (int) ($row['patient_id'] ?? 0);
                $disFaId = (int) ($row['facility_admission_id'] ?? 0);
            }
            mysqli_begin_transaction($connection);
            ...
            if ($ok) {
                if ($disFaId > 0) {
                    hms_facility_admission_close_by_id($connection, $disFaId);
                }
                if ($disPatientId > 0) {
                    hms_patient_sync_type_from_admission_state($connection, $fid, $disPatientId);
                }
                mysqli_commit($connection);
    ...
    } elseif (isset($_POST['bed_delete'])) {
        $bedId = (int) ($_POST['bed_id'] ?? 0);
        if ($bedId > 0) {
            $stmt = mysqli_prepare(
                $connection,
                "DELETE FROM tbl_bed WHERE id = ? AND facility_id = ? AND status = 'available'"
            );
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, 'ii', $bedId, $fid);
                mysqli_stmt_execute($stmt);
                $n = mysqli_stmt_affected_rows($stmt);
                mysqli_stmt_close($stmt);
                $_SESSION['adt_flash'] = $n > 0 ? 'Bed deleted.' : 'Bed could not be deleted (must be available).';
            }
        }
    } elseif (isset($_POST['bed_edit'])) {
        $bedId = (int) ($_POST['bed_id'] ?? 0);
        $newLabel = trim((string) ($_POST['new_bed_label'] ?? ''));
        if ($bedId > 0 && $newLabel !== '') {
            $stmt = mysqli_prepare(
                $connection,
                "UPDATE tbl_bed SET bed_label = ? WHERE id = ? AND facility_id = ? AND status = 'available'"
            );
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, 'sii', $newLabel, $bedId, $fid);
                mysqli_stmt_execute($stmt);
                $n = mysqli_stmt_affected_rows($stmt);
                mysqli_stmt_close($stmt);
                $_SESSION['adt_flash'] = $n > 0 ? 'Bed updated.' : 'Bed could not be updated (must be available).';
            }
        }
    }
...
?>
        <div class="page-wrapper">
            <div class="content hms-module">
                <?php
                $toolbar = [
                    'subtitle' => 'Facility admission = patient arrived on site. Hospitalization = bed assigned (inpatient stay; post daily charges against the hospitalization id). Transfers and discharge update the bed board.',
                ];
                $adtSec = [];
                ...
                hms_ui_page_header('Ward & Bed MGT', $toolbar);
                ?>
                ...
                <form method="post" id="drag_transfer_form" style="display:none;">
                    <?php echo hms_csrf_field(); ?>
                    <input type="hidden" name="admission_id" id="drag_transfer_admission_id">
                    <input type="hidden" name="to_bed_id" id="drag_transfer_to_bed_id">
                    <input type="hidden" name="transfer_admission" value="1">
                </form>
                ...
                <div class="mb-4">
                    ...
                    <div class="row">
                        <?php foreach ($bedsInWard as $b) {
                            ...
                            $isDraggable = ($canWrite && $admId > 0 && strtolower($st) === 'occupied') ? 'true' : 'false';
                            $onDragStart = $isDraggable === 'true' ? "ondragstart=\"event.dataTransfer.setData('text/plain', '{$admId}'); event.currentTarget.style.opacity = '0.5';\"" : "";
                            $onDragEnd = $isDraggable === 'true' ? "ondragend=\"event.currentTarget.style.opacity = '1';\"" : "";

                            $isDroppable = ($canWrite && strtolower($st) === 'available');
                            $onDragOver = $isDroppable ? "ondragover=\"event.preventDefault(); event.currentTarget.classList.add('border-primary'); event.currentTarget.classList.add('shadow');\" ondragleave=\"event.currentTarget.classList.remove('border-primary'); event.currentTarget.classList.remove('shadow');\"" : "";
                            $onDrop = $isDroppable ? "ondrop=\"event.preventDefault(); event.currentTarget.classList.remove('border-primary'); var srcAdm = event.dataTransfer.getData('text/plain'); if(srcAdm){ document.getElementById('drag_transfer_admission_id').value = srcAdm; document.getElementById('drag_transfer_to_bed_id').value = '{$bid}'; document.getElementById('drag_transfer_form').submit(); }\"" : "";

                            $losHtml = '';
                            if ($admId > 0 && !empty($b['admitted_at'])) {
                                $admitTime = strtotime((string)$b['admitted_at']);
                                $nowTime = time();
                                $days = floor(($nowTime - $admitTime) / 86400);

                                $expectedLos = 3; 
                                $dx = strtolower((string)($b['admitting_diagnosis'] ?? ''));
                                if (strpos($dx, 'surgery') !== false || strpos($dx, 'fracture') !== false) {
                                    $expectedLos = 5;
                                } elseif (strpos($dx, 'maternity') !== false || strpos($dx, 'labor') !== false || strpos($dx, 'labour') !== false || strpos($dx, 'delivery') !== false) {
                                    $expectedLos = 2;
                                }
                                
                                if ($days >= $expectedLos) {
                                    $losHtml = '<div class="mt-2 text-danger font-weight-bold small px-2 py-1 bg-white border border-danger rounded"><i class="fa fa-warning"></i> LoS: ' . $days . ' / ' . $expectedLos . ' Days Expected!</div>';
                                } else {
                                    $losHtml = '<div class="mt-2 text-muted small"><i class="fa fa-clock-o"></i> LoS: ' . $days . ' Day(s) (Expected ' . $expectedLos . ')</div>';
                                }
                            }
                            ?>
                        <div class="col-sm-6 col-lg-4 col-xl-3 mb-3">
                            <div class="card hms-bed-tile border shadow-sm h-100" <?php echo $isDraggable==='true'?'draggable="true"':''; ?> <?php echo $onDragStart; ?> <?php echo $onDragEnd; ?> <?php echo $onDragOver; ?> <?php echo $onDrop; ?> style="transition: all 0.2s; <?php echo $isDraggable==='true'?'cursor:grab;':''; ?>">
                                <div class="card-body d-flex flex-column">
                                    ...
                                    <div class="font-weight-bold mb-1"><?php echo hms_h($pt); ?></div>
                                    <div class="small text-muted mb-1">Since <?php echo hms_h((string) ($b['admitted_at'] ?? '')); ?></div>
                                    <?php echo $losHtml; ?>
                                    <div class="mb-3"></div>
                                    <?php } else { ?>
                                    ...
                                    <div class="mt-auto">
                                        ...
                                        <div class="d-flex">
                                            <button type="button" class="btn btn-sm btn-outline-info flex-fill mr-1" data-toggle="modal" data-target="#adtEditBedModal" data-bed-id="<?php echo $bid; ?>" data-bed-label="<?php echo hms_h((string) $b['bed_label']); ?>" title="Edit bed"><i class="fa fa-pencil"></i></button>
                                            <form method="post" class="mb-0 flex-fill mr-1" onsubmit="return confirm('Delete this bed forever?');" style="display:flex;">
                                                <?php echo hms_csrf_field(); ?>
                                                <input type="hidden" name="bed_id" value="<?php echo $bid; ?>">
                                                <button type="submit" name="bed_delete" class="btn btn-sm btn-outline-danger w-100" title="Delete bed"><i class="fa fa-trash"></i></button>
                                            </form>
                                            <form method="post" class="mb-0 flex-fill" onsubmit="return confirm('Block this bed for maintenance?');" style="display:flex;">
                                                <?php echo hms_csrf_field(); ?>
                                                <input type="hidden" name="bed_id" value="<?php echo $bid; ?>">
                                                <button type="submit" name="bed_block" class="btn btn-sm btn-outline-warning w-100" title="Block bed"><i class="fa fa-ban"></i></button>
                                            </form>
                                        </div>
                ...
                <div class="modal fade" id="adtEditBedModal" tabindex="-1" role="dialog" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form method="post">
                                <?php echo hms_csrf_field(); ?>
                                <input type="hidden" name="bed_id" id="adt_edit_bed_id" value="">
                                <div class="modal-header">
                                    <h5 class="modal-title">Edit Bed <span id="adt_edit_bed_label_display"></span></h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                </div>
                                <div class="modal-body">
                                    <div class="form-group">
                                        <label for="adt_new_bed_label">New Bed Label</label>
                                        <input type="text" class="form-control" name="new_bed_label" id="adt_new_bed_label" required maxlength="32">
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                                    <button type="submit" name="bed_edit" class="btn btn-primary">Save Changes</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
(function () {
  ...
  $('#adtEditBedModal').on('show.bs.modal', function (e) {
    var btn = $(e.relatedTarget);
    $('#adt_edit_bed_id').val(btn.data('bed-id') || '');
    var label = btn.data('bed-label') || '';
    $('#adt_edit_bed_label_display').text(label);
    $('#adt_new_bed_label').val(label);
  });

...
$receiptBatch = 500;
$expenseBatch = 500;
$rfD1 = trim((string) ($_POST['rf_d1'] ?? $_GET['rf_d1'] ?? ''));
$rfD2 = trim((string) ($_POST['rf_d2'] ?? $_GET['rf_d2'] ?? ''));
$exD1 = trim((string) ($_POST['ex_d1'] ?? $_GET['ex_d1'] ?? ''));
$exD2 = trim((string) ($_POST['ex_d2'] ?? $_GET['ex_d2'] ?? ''));
...
if ($exD1 === '' && $exD2 === '') {
    if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $rfD1) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $rfD2)) {
        $exD1 = $rfD1;
        $exD2 = $rfD2;
    } else {
        $exD1 = date('Y-m-01');
...
if ($finOk && $canRun && ($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    if (!hms_csrf_validate($_POST['hms_csrf'] ?? null)) {
        ...
        $rIns = (int) ($r['inserted'] ?? 0);
        $eIns = (int) ($e['inserted'] ?? 0);
        $rProc = (int) ($r['processed'] ?? 0);
        $eProc = (int) ($e['processed'] ?? 0);
        if ($rIns === 0 && $eIns === 0 && ($rProc > 0 || $eProc > 0)) {
            $msg .= ' Those operational rows already have matching GL journals (nothing new to insert). '
            ...
            $spanQ = @mysqli_query(
                $connection,
                'SELECT MIN(entry_date) AS a, MAX(entry_date) AS b FROM tbl_fin_journal_header WHERE facility_id = ' . (int) $fid
            );
            if ($spanQ && ($spanRow = mysqli_fetch_assoc($spanQ))) {
                mysqli_free_result($spanQ);
                $a = trim((string) ($spanRow['a'] ?? ''));
                $b = trim((string) ($spanRow['b'] ?? ''));
                if ($a !== '' && $b !== '') {
                    $msg .= ' On site #' . (int) $fid . ', journal entry_date ranges from ' . $a . ' through ' . $b . ' — set reports to include that range.';
                }
            }
        }
    }
